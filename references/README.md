# 配套阅读资料

## 《深入理解 AI Agent：设计原理与工程实践》

- 文件：[`AI-Agents-in-Depth-zh-CN.pdf`](AI-Agents-in-Depth-zh-CN.pdf)
- 版本：v1.3
- 日期：2026-07-27
- 作者：李博杰
- 上游仓库：[bojieli/ai-agent-book](https://github.com/bojieli/ai-agent-book)
- 在线阅读：[AI Agents in Depth](https://bojieli.github.io/ai-agent-book/)

本 PDF 是本仓库 14 周学习路线的主教材。学习笔记应优先链接到书中章节、页码或上游 Markdown 源文件，避免复制大段原文。

## 许可与归属

该书及其配套材料的版权与许可证归原作者及上游项目所有。上游仓库声明采用 Apache License 2.0；本仓库仅将 PDF 作为学习资料随附，不改变其原有归属或许可。

本仓库自行创建的代码、笔记、模板和项目文档仍采用根目录的 MIT License。若要再分发、修改或摘录本 PDF，请先核对上游仓库中的最新许可证和署名要求。

